...
namespace ds::amt {
	...
	{
		BlockType* parent = this->accessParent(node);
		return parent == nullptr ?  false :  this->accessSon(*parent, sonOrder) == &node;
	}
    ...
    void Hierarchy<BlockType>::processPreOrder(const BlockType* node, std::function<void(const BlockType*)> operation) const
	{
		...
		size_t sonIndex = 0;
		size_t sonsProccessed = 0;
		size_t degree = this->degree(*node);

		while (sonsProccessed < degree)
		{
			BlockType* son = this->accessSon(*node, sonIndex);
			if (son != nullptr)
			{
				this->processPreOrder(this->accessSon(*node, i), operation);
				++sonsProccessed;
			}
			++sonIndex;
		}
